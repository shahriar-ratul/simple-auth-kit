...
export const LanguagesPage = observer(function LanguagesPage() {
  ...
  // Keyed on the active workspace, same as `UsersPage`: `listLanguages` is scoped to it
  // automatically via `bindActiveWorkspace`, so switching workspaces has to refetch or the table
  // would keep showing one workspace's languages under another's name.
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ["languages-list", activeWorkspaceId, pagination.pageIndex, pagination.pageSize, searchKey, status],
    queryFn: () =>
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <Link to={`/languages/${row.original.id}`} className="font-medium hover:underline">
              {row.original.name}
            </Link>
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex justify-end items-center gap-1.5">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link to={`/languages/${language.id}`} className={cn(buttonVariants({ variant: "outline", size: "icon" }))}>
                    <EyeIcon />
                  </Link>
                </TooltipTrigger>
              ...
              <Tooltip>
                ...
                <TooltipContent>{canManage ? "Delete language" : `You need the "${PERMISSIONS.languagesManage}" permission to do this.`}</TooltipContent>
              </Tooltip>
            </div>
          );
        },
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {canManage ? (
            <Link to="/languages/new" className={cn(buttonVariants({ size: "sm" }))}>
              <PlusIcon />
              Add language
            </Link>
          ) : null}
        ...
        <CardContent className="flex flex-col gap-4">
          {isError ? <p className="text-sm text-destructive">{apiErrorMessage(error, "Couldn't load languages. Try again.")}</p> : null}
