...
export const CustomerDetailPage = observer(function CustomerDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setCustomer(await authClient.getCustomer(id));
    } catch (err) {
  ...
  // Keyed on the active workspace, same as `CustomersPage`: the record below is whatever this
  // workspace says about the customer, so switching has to re-fetch rather than show stale data.
  useEffect(() => {
    void load();
...
function DeleteCustomerDialog({
  ...
  async function handleDelete() {
    setSubmitting(true);
    try {
      await authClient.deleteCustomer(customerId);
  ...
  return (
    <Modal
      isOpen={open}
      onClose={onClose}
      title={`Delete ${email || "this customer"}?`}
      description="This soft-deletes the customer: they stop appearing in listings, but the row is kept for audit purposes."
    >
      <div className="flex flex-col gap-4">
