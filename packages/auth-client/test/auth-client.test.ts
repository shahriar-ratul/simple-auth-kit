...
function everyCall(
  c: AuthClient,
): Array<{ name: string; scoped: boolean; run: () => Promise<unknown> }> {
  return [
    {
      name: "signup",
      scoped: false,
      run: () => c.signup({ email: "a@example.com", password: "pw" }),
    },
    {
      name: "login",
      scoped: false,
      run: () => c.login({ identifier: "a@example.com", password: "pw" }),
    },
    {
      name: "loginTwoFactor",
      scoped: false,
      run: () => c.loginTwoFactor({ challengeToken: "chal-1", code: "123456" }),
    },
    ...
    { name: "logoutOthers", scoped: false, run: () => c.logoutOthers() },
    {
      ...
      scoped: false,
      run: () =>
        c.changePassword({ currentPassword: "old-pw", newPassword: "new-pw" }),
    },
    ...
    { name: "enrollTwoFactor", scoped: false, run: () => c.enrollTwoFactor() },
    {
      ...
      scoped: false,
      run: () => c.confirmTwoFactor("123456"),
    },
    {
      name: "disableTwoFactor",
      scoped: false,
      run: () => c.disableTwoFactor("123456"),
    },
    {
      name: "requestPasswordReset",
      scoped: false,
      run: () => c.requestPasswordReset({ email: "a@example.com" }),
    },
    {
      name: "resetPassword",
      scoped: false,
      run: () => c.resetPassword({ token: "t", newPassword: "pw2" }),
    },
    { name: "oauthStart", scoped: false, run: () => c.oauthStart("google") },
    {
      name: "createWorkspace",
      scoped: false,
      run: () => c.createWorkspace("Acme Inc"),
    },
    ...
    { name: "me", scoped: true, run: () => c.me() },
    {
      ...
      scoped: true,
      run: () => c.listWorkspaceMembers(),
    },
    {
      name: "addWorkspaceMember",
      scoped: true,
      run: () => c.addWorkspaceMember({ email: "b@example.com" }),
    },
    {
      name: "setWorkspaceMemberRoles",
      scoped: true,
      run: () => c.setWorkspaceMemberRoles("m1", ["admin"]),
    },
    {
      name: "removeWorkspaceMember",
      scoped: true,
      run: () => c.removeWorkspaceMember("m1"),
    },
    ...
    { name: "unblockUser", scoped: true, run: () => c.unblockUser("u1") },
    {
      name: "listUsers",
      scoped: true,
      run: () => c.listUsers({ search: "a" }),
    },
    {
      name: "createUser",
      scoped: true,
      run: () => c.createUser({ email: "new@example.com", password: "pw" }),
    },
    { name: "getUser", scoped: true, run: () => c.getUser("u1") },
    {
      name: "updateUser",
      scoped: true,
      run: () => c.updateUser("u1", { displayName: "Ada" }),
    },
    ...
    { name: "listRoles", scoped: true, run: () => c.listRoles() },
    {
      name: "createRole",
      scoped: true,
      run: () => c.createRole({ slug: "billing-manager" }),
    },
    {
      name: "updateRole",
      scoped: true,
      run: () => c.updateRole("role-1", { displayName: "Billing" }),
    },
    { name: "deleteRole", scoped: true, run: () => c.deleteRole("role-1") },
    {
      name: "attachPermissionToRole",
      scoped: true,
      run: () => c.attachPermissionToRole("role-1", "billing:manage"),
    },
    {
      name: "assignRole",
      scoped: true,
      run: () => c.assignRole("u1", "billing-manager"),
    },
    {
      name: "revokeRole",
      scoped: true,
      run: () => c.revokeRole("u1", "billing-manager"),
    },
    {
      name: "grantPermission",
      scoped: true,
      run: () => c.grantPermission("u1", "billing:manage"),
    },
    {
      name: "revokePermission",
      scoped: true,
      run: () => c.revokePermission("u1", "billing:manage"),
    },
    { name: "listPermissions", scoped: true, run: () => c.listPermissions() },
    {
      name: "definePermission",
      scoped: true,
      run: () => c.definePermission({ slug: "billing:manage" }),
    },
    {
      name: "listAuditLog",
      scoped: true,
...
describe("AuthClient", () => {
  ...
  describe("identity", () => {
    it("login stores tokens on a direct success", async () => {
      ...
      expect(storage._stored).toEqual({
        accessToken: "a1",
        refreshToken: "r1",
        sessionId: "s1",
      });
    ...
    it("signup posts email and password and nothing else", async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toBe(
        "https://api.example.com/api/v1/auth/signup",
      );
      expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual({
        email: "a@example.com",
        password: "pw",
    ...
    it("login returns the 2FA challenge without storing tokens", async () => {
      fetchMock.mockResolvedValueOnce(
        jsonResponse({ twoFactorRequired: true, challengeToken: "chal-1" }),
      );
    ...
    it("me() hands back fields this version does not know about", async () => {
      // `/api/v1/auth/me` is about to grow a `rules` field (CASL rules serialized with `packRules`), so
    ...
    it("oauthStart fetches the provider URL from the backend", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        "https://api.example.com/api/v1/auth/oauth/google/start",
  ...
  describe("refresh-on-401", () => {
    ...
    it("does not retry more than once", async () => {
      ...
      fetchMock
        ...
        .mockResolvedValueOnce(
          jsonResponse({ message: "still unauthorized" }, 401),
        );
    ...
    it("throws AuthApiError with the parsed message and code on a non-2xx response", async () => {
      ...
      const error = await client
        .signup({ email: "dup@example.com", password: "pw" })
        .catch((e) => e);
    ...
    it("sends no field the contract does not name", async () => {
      ...
      for (const [url, init] of fetchMock.mock.calls) {
        ...
        for (const key of new URL(url).searchParams.keys()) {
          expect(QUERY_FIELDS, `${url} sent query param "${key}"`).toContain(
            key,
          );
        }
    ...
    it("never names the removed tenancy concept, anywhere, on any call", async () => {
      ...
      for (const [url, init] of fetchMock.mock.calls) {
        expect(
          ...
          `request to ${url}`,
        ).not.toMatch(new RegExp(REMOVED, "i"));
      }
    });
  });
  ...
  describe("X-Workspace-Id", () => {
    it("is never sent by a consumer that configured no workspace (the plain backend variant)", async () => {
      ...
      for (const [url, init] of fetchMock.mock.calls) {
        expect(
          ...
          `header sent to ${url}`,
        ).not.toContain(WORKSPACE_HEADER);
      }
    });
      ...
      for (const call of everyCall(scoped)) {
        ...
        if (call.scoped)
          expect(
            ...
            `${call.name} must name its workspace`,
          ).toBe("ws-1");
        else
          expect(
            headers[WORKSPACE_HEADER],
            `${call.name} must not name a workspace`,
          ).toBeUndefined();
      }
    });
    ...
    it("is absent from every identity endpoint, named one by one", async () => {
      ...
      const identityCalls: Array<[string, () => Promise<unknown>]> = [
        [
          "/api/v1/auth/signup",
          () => scoped.signup({ email: "a@example.com", password: "pw" }),
        ],
        [
          "/api/v1/auth/login",
          () => scoped.login({ identifier: "a@example.com", password: "pw" }),
        ],
        [
          "/api/v1/auth/login/2fa",
          () => scoped.loginTwoFactor({ challengeToken: "c", code: "1" }),
        ],
        ["/api/v1/auth/refresh", () => scoped.refresh()],
        ["/api/v1/auth/logout", () => scoped.logout()],
        ["/api/v1/auth/logout-all", () => scoped.logoutAll()],
        ["/api/v1/auth/logout-others", () => scoped.logoutOthers()],
        ["/api/v1/auth/sessions", () => scoped.sessions()],
        ["/api/v1/auth/2fa/enroll", () => scoped.enrollTwoFactor()],
        ["/api/v1/auth/2fa/confirm", () => scoped.confirmTwoFactor("123456")],
        ["/api/v1/auth/2fa/disable", () => scoped.disableTwoFactor("123456")],
        [
          "/api/v1/auth/password/forgot",
          () => scoped.requestPasswordReset({ email: "a@example.com" }),
        ],
        [
          "/api/v1/auth/password/reset",
          () => scoped.resetPassword({ token: "t", newPassword: "pw2" }),
        ],
        ["/api/v1/auth/oauth/google/start", () => scoped.oauthStart("google")],
      ];
      ...
      for (const [path, run] of identityCalls) {
        ...
        expect(
          headersOf(0)[WORKSPACE_HEADER],
          `${path} must not name a workspace`,
        ).toBeUndefined();
      }
    });
  ...
  describe("workspace endpoints", () => {
    ...
    it("listWorkspaceMembers GETs /workspaces/members", async () => {
      ...
      const [url, init] = fetchMock.mock.calls[0];
      expect([url, init.method]).toEqual([
        "https://api.example.com/api/v1/workspaces/members",
        "GET",
      ]);
    });
    ...
    it("addWorkspaceMember POSTs the email, and omits roles when not given", async () => {
      ...
      await scoped.addWorkspaceMember({
        email: "c@example.com",
        roles: ["admin", "member"],
      });
    ...
    it("setWorkspaceMemberRoles PUTs the whole role set to the member's path", async () => {
      fetchMock.mockResolvedValueOnce(
        jsonResponse({ memberId: "m 1", roles: ["admin"] }),
      );
      ...
      const [url, init] = fetchMock.mock.calls[0];
      expect([url, init.method]).toEqual([
        "https://api.example.com/api/v1/workspaces/members/m%201/roles",
        "PUT",
      ]);
    ...
    it("removeWorkspaceMember DELETEs the member's path with no body", async () => {
      ...
      const [url, init] = fetchMock.mock.calls[0];
      expect([url, init.method, init.body]).toEqual([
        "https://api.example.com/api/v1/workspaces/members/m1",
        "DELETE",
        undefined,
      ]);
    });
  ...
  describe("administration", () => {
    ...
    it("listAuditLog serializes filters as query params, keyed on `action`", async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toBe(
        "https://api.example.com/api/v1/audit-log?userId=u1&action=role_assigned&limit=10",
      );
    ...
    it("listUsers serializes its filter", async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toBe(
        "https://api.example.com/api/v1/admin/users?search=ali&limit=5&page=2",
      );
    });
    ...
    it("createRole and assignRole send the bare backend bodies", async () => {
      ...
      expect(fetchMock.mock.calls[1][0]).toBe(
        "https://api.example.com/api/v1/admin/users/u1/roles",
      );
      expect(JSON.parse(fetchMock.mock.calls[1][1].body)).toEqual({
    ...
    it("block/unblock and the revoke routes encode their path segments", async () => {
      ...
      expect(fetchMock.mock.calls[0][0]).toBe(
        "https://api.example.com/api/v1/admin/users/u%2F1/block",
      );
      expect(fetchMock.mock.calls[1][0]).toBe(
        "https://api.example.com/api/v1/admin/users/u1/roles/billing%20manager/revoke",
      );
      expect(fetchMock.mock.calls[2][0]).toBe(
        "https://api.example.com/api/v1/admin/users/u1/permissions/billing%3Amanage/revoke",
      );
    });
  });
