...
import { checkRateLimit, type RateLimitDeps } from '@/core/rate-limit';
import {
  createSession,
  revokeAccessToken,
...
//
// Identity endpoints only — signup/login/session lifecycle, 2FA, password reset, self-profile.
// Member/role/permission administration lives in AdminService/RolesService/PermissionsService,
// and the audit log in AuditLogService (see modules/admin, modules/roles, modules/permissions,
// modules/audit-log). `getProfile`/`updateProfile` below are deliberately not workspace-scoped
// (a profile belongs to the account, not to any one membership), so this class needs neither
// `RbacRepository` nor `AuditLogRepository` as a dependency — unlike the base variant's
// `AuthService`, which keeps `rbac` for that same self-service path.
export class AuthService {
  ...
  async login(input: {
    ...
    const [user] = await this.db
      ...
      .from(users)
      .where(
        or(eq(users.email, input.identifier), eq(users.username, input.identifier), eq(users.phone, input.identifier)),
      )
      .limit(1);
    if (!user || user.blocked || !user.isActive || user.isDeleted || !user.passwordHash)
  ...
  async loginTwoFactor(input: {
    ...
    const [user] = await this.db
      ...
      .limit(1);
    if (!user || user.blocked || !user.isActive || user.isDeleted || !user.twoFactorEnabled || !user.twoFactorSecret)
      throw new HttpError(401, 'invalid credentials');
