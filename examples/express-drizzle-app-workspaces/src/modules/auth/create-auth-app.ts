 ...
 * then mounting `express.json()` + the auth, admin, roles, permissions, audit-log and workspace
 * routers. Equivalent of the reference combo's `AuthModule.forRoot(...)`, just a function instead
 * of a NestJS dynamic module — there's no DI container here, so wiring is explicit constructor
 * calls in one place.
 *
...
export function createAuthApp(config: AuthConfigInput = {}): Express {
  ...
  const authService = new AuthService(db, sessions, keys, rateLimit, twoFactor, oauth, passwordReset, resolvedConfig);
  const adminService = new AdminService(db, sessions, auditLog, rbac);
