...
export const users = pgTable(
  'users',
  {
    ...
    createdBy: bigint('created_by', { mode: 'bigint' }).references((): AnyPgColumn => users.id, {
      onDelete: 'set null',
    }),
    updatedBy: bigint('updated_by', { mode: 'bigint' }).references((): AnyPgColumn => users.id, {
      onDelete: 'set null',
    }),
    isDeleted: boolean('is_deleted').notNull().default(false),
    deletedAt: timestamptz('deleted_at'),
    deletedBy: bigint('deleted_by', { mode: 'bigint' }).references((): AnyPgColumn => users.id, {
      onDelete: 'set null',
    }),
    deletedReason: varchar('deleted_reason', { length: 255 }),
    createdAt: timestamptz('created_at').notNull().defaultNow(),
...
export const sessions = pgTable(
  'sessions',
  {
    ...
    revokedBy: bigint('revoked_by', { mode: 'bigint' }).references((): AnyPgColumn => users.id, {
      onDelete: 'set null',
    }),
