...
export class CountryRepository {
  ...
  async get(countryId: string): Promise<CountrySummary> {
    const row = await this.prisma.country.findUnique({ where: { id: toId(countryId), isDeleted: false } });
  ...
  async create(input: CountryInput, actorUserId: string | null): Promise<CountrySummary> {
    const existing = await this.prisma.country.findFirst({ where: { OR: [{ code: input.code }, { isoCode: input.isoCode }] } });
  ...
  async update(countryId: string, input: Partial<CountryInput>, actorUserId: string | null): Promise<CountrySummary> {
    ...
    const existing = await this.prisma.country.findUnique({ where: { id: countryIdBig, isDeleted: false }, select: { id: true } });
    ...
    if (input.code !== undefined || input.isoCode !== undefined) {
      const clash = await this.prisma.country.findFirst({
        where: { NOT: { id: countryIdBig }, OR: [{ code: input.code }, { isoCode: input.isoCode }] },
      });
