...
export class RoleController {
  ...
  async deleteRole(@Param('roleId') roleId: string, @Body() body: Record<string, unknown>, @Req() req: Request) {
    await this.roles.deleteRole(roleId, req.auth!.sub, optionalString(body?.reason));
    return { ok: true };
  }
  ...
  async attachPermissionToRole(
    ...
    await this.roles.attachPermissionToRole(roleId, requireString(body.permission, 'permission'), req.auth!.sub);
    return { ok: true };
  }
