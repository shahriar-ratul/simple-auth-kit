...
export type UserRow = Prisma.UserGetPayload<{
  include: typeof USER_ROLES_INCLUDE;
}>;
...
export class RbacRepository {
  ...
  async createUser(
    ...
    if (input.roles && input.roles.length > 0) {
      const roles = await this.prisma.role.findMany({
        where: { slug: { in: input.roles }, isActive: true, isDeleted: false },
        select: { id: true },
      });
      if (roles.length > 0) {
  ...
  async assignRoleToUser(userId: string, roleSlug: string): Promise<void> {
    const userIdBig = toId(userId);
    const role = await this.prisma.role.findUnique({
      where: { slug: roleSlug, isDeleted: false },
      select: { id: true },
    });
  ...
  async grantPermissionToUser(userId: string, permissionSlug: string, actorUserId: string | null): Promise<void> {
    ...
    await this.prisma.permissionUser.upsert({
      where: {
        userId_permissionId: { userId: userIdBig, permissionId: permission.id },
      },
