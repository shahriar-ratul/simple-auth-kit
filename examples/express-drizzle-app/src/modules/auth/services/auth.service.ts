...
// User/role/permission administration lives in AdminService/RolesService/PermissionsService,
// and the audit log in AuditLogService (see modules/admin, modules/roles, modules/permissions,
// modules/audit-log). `rbac` stays a dependency here too: `getProfile`/`updateProfile` are
// self-service reads/writes on the same `RbacRepository`, which is not split — see the note on
// `createAdminRouter`.
