...
export function createRolesRouter(deps: RolesRouterDeps): RequestHandler {
  ...
  router.route('get', '/', ability('roles:manage'), async (_req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  router.route(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const body = (req.body ?? {}) as Record<string, unknown>;
        await roles.deleteRole(requireString(req.params.roleId, 'roleId'), req.auth!.sub, optionalString(body.reason));
