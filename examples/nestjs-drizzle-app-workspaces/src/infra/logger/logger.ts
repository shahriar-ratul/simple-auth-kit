...
export type LogModule = 'auth' | 'admin' | 'roles' | 'permissions' | 'audit-log';

type LogLevel = 'debug' | 'info' | 'warn' | 'error';

const MODULES: readonly LogModule[] = ['auth', 'admin', 'roles', 'permissions', 'audit-log'];
...
class FileLoggerRegistry {
  ...
  private moduleLogger(level: LogLevel, module: LogModule): Logger {
    ...
    if (!logger) {
      ...
      logger = pino(PINO_OPTIONS, pino.destination({ dest, mkdir: true, sync: false }));
  ...
  write(level: LogLevel, module: LogModule, msg: string, meta?: Record<string, unknown>): void {
    const traceId = requestContext.getStore()?.traceId;
    const entry = traceId && !meta?.['traceId'] ? { traceId, ...meta } : (meta ?? {});
...
export function deriveModule(path: string): LogModule | undefined {
  ...
  return segments.find((segment): segment is LogModule => (MODULES as readonly string[]).includes(segment));
}

/** Any key matching this — at any nesting depth — has its value masked before it reaches a log file. */
const SENSITIVE_KEY_PATTERN = /password|secret|token|authorization|api[-_]?key|credential|otp|totp/i;
