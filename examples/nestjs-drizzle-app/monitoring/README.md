...
The application exposes Prometheus metrics at `GET /metrics`, wired up by `setupMetrics(app)` in your `main.ts`, next to `setupDocs(app)`.
...
|               |                                                                          |
| ------------- | ------------------------------------------------------------------------ |
| Grafana       | <http://localhost:3032> — dashboard provisioned, nothing to import       |
| Prometheus    | <http://localhost:9092> — _Status > Target health_ when a panel is empty |
| node_exporter | <http://localhost:9102/metrics>                                          |

Prometheus reaches the application at `host.docker.internal:3002`, because the app runs on
