...
function nameOf(p: ProfileFields): string | null {
  return (
    p.displayName || [p.firstName, p.lastName].filter(Boolean).join(" ") || null
  );
}
...
export class AuthzCache {
  ...
  async get<T>(
    ...
    if (value !== null)
      await store.set(storeKey, JSON.stringify({ version, value }), ttlSeconds);
    return value;
  }
  ...
  async getProfile<T extends object>(
    ...
    await store.set(
      ...
      this.config.authzCache.ttlSeconds,
    );
    return profile;
  }
  ...
  async inspect(workspaceId?: string): Promise<AuthzCacheInspection> {
    ...
    const rows = store?.list
      ? (await store.list(prefix)).filter((row) => row.key.startsWith(prefix))
      : null;
    return {
      active: this.active,
      enabled,
      ...
      stats: { ...this.stats },
      entries:
        rows && (await this.describe(rows, prefix, version, workspaceId)),
    };
  }
  ...
  private async describe(
    ...
    if (store)
      for (const userId of new Set(userIds)) {
        ...
        if (raw !== undefined)
          cached.set(userId, JSON.parse(raw) as ProfileFields);
      }
