...
/**
 * Administration *of one workspace*, named by the `X-Workspace-Id` header: user management,
 * block/unblock/deactivate/activate, and user-scoped role/permission assignment. Role/permission
 * *catalog* management (create a role, define a permission) lives in
 * `modules/roles`/`modules/permissions` instead; the audit log has its own `modules/audit-log`.
 *
 * Authority is a permission, never a role name: the workspace middleware resolves the caller's
 * membership and the roles and permissions it carries **in that workspace**, builds the CASL
 * ability from them, and each route's `ability(...)` tier checks its slugs against it. There is no
 * role-based bypass — holding a role called "admin" that carries no permissions gets exactly the
 * same 403 as holding no role at all, which is why `npm run seed` (which provisions the catalog
 * and the roles that carry it) is a prerequisite for this router doing anything.
 *
 ...
 *
 * Replaces the reference combo's `AdminController`. Mounted at `/api/v1/admin` by
