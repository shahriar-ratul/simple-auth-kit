...
export const workspaces = pgTable(
  ...
  (table) => [
    uniqueIndex("workspaces_uuid_key").on(table.uuid),
    index("workspaces_is_deleted_idx").on(table.isDeleted),
  ],
);
...
export const workspaceMembers = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("workspace_members_user_id_workspace_id_key").on(
      table.userId,
      table.workspaceId,
    ),
...
export const users = pgTable(
  "users",
  {
    ...
    isDeleted: boolean("is_deleted").notNull().default(false),
    deletedAt: timestamptz("deleted_at"),
    deletedBy: bigint("deleted_by", { mode: "bigint" }).references(
      (): AnyPgColumn => users.id,
      { onDelete: "set null" },
    ),
    deletedReason: varchar("deleted_reason", { length: 255 }),
...
export const roles = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("roles_workspace_id_slug_key").on(
      table.workspaceId,
      table.slug,
    ),
...
export const permissions = pgTable(
  ...
  (table) => [
    ...
    index("permissions_group_order_idx").on(
      table.group,
      table.groupOrder,
      table.order,
    ),
...
export const roleMember = pgTable(
  ...
  (table) => [
    primaryKey({
      name: "role_member_pkey",
      columns: [table.memberId, table.roleId],
    }),
    index("role_member_role_id_idx").on(table.roleId),
  ],
);
...
export const permissionMember = pgTable(
  ...
  (table) => [
    primaryKey({
      name: "permission_member_pkey",
      columns: [table.memberId, table.permissionId],
    }),
...
export const oauthAccounts = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("oauth_accounts_provider_provider_account_id_key").on(
      table.provider,
...
export const passwordResetTokens = pgTable(
  ...
  (table) => [
    uniqueIndex("password_reset_tokens_uuid_key").on(table.uuid),
...
export const twoFactorBackupCodes = pgTable(
  ...
  (table) => [
    uniqueIndex("two_factor_backup_codes_uuid_key").on(table.uuid),
    index("two_factor_backup_codes_user_id_idx").on(table.userId),
  ],
);
...
export const auditLogs = pgTable(
  "audit_logs",
  {
    id: id(),
    uuid: uuid(),
    workspaceId: bigint("workspace_id", { mode: "bigint" }).references(
      () => workspaces.id,
    ),
...
export const permissionMemberRelations = relations(
  permissionMember,
  ({ one }) => ({
    member: one(workspaceMembers, {
      fields: [permissionMember.memberId],
      references: [workspaceMembers.id],
    }),
    permission: one(permissions, {
      fields: [permissionMember.permissionId],
      references: [permissions.id],
    }),
  }),
);
