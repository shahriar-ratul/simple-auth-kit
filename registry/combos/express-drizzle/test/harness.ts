...
/**
 * Every route across the admin surface — `/admin`, `/roles`, `/permissions`, `/audit-log` —
 * paired with its `@CheckAbility` slug. Both variants expose exactly this surface — the
 * workspace variant adds `/workspaces/*` routes on top, which its own hooks cover, so this list
