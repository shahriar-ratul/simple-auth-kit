...
export const AuditLogPage = observer(function AuditLogPage() {
  ...
  async function runQuery(page: number) {
    setLoading(true);
    setError(null);
    try {
  ...
  // A switch invalidates every loaded row — the backend scopes this query by `X-Workspace-Id`,
  // so results from the previous workspace must not linger under the new one's name.
  useEffect(() => {
    setEntries([]);
    setMeta(undefined);
    setLoadedOnce(false);
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Everything security-relevant that happened in {workspaceName}: blocks, role and permission changes, member changes. Account-level events
          such as logins belong to no workspace and don't appear here.
        </p>
      </div>
              ...
              {meta && meta.total > 0 ? (
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" disabled={!meta.hasPreviousPage || loading} onClick={() => runQuery(meta.page - 1)}>
                      Previous
                    </Button>
                    <span className="text-sm text-muted-foreground">
                      Page {meta.page} of {meta.pageCount}
                    </span>
                    <Button variant="outline" size="sm" disabled={!meta.hasNextPage || loading} onClick={() => runQuery(meta.page + 1)}>
                      Next
                    </Button>
                  </div>
                </div>
