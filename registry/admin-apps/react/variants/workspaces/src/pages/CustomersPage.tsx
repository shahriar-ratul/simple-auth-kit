...
export const CustomersPage = observer(function CustomersPage() {
  const ability = useAbility();
  const workspaceStore = useWorkspaceStore();
  const activeWorkspaceId = workspaceStore.activeWorkspaceId;
  const canManage = ability.can(PERMISSIONS.customersManage, "permission");
  const canStatus = ability.can(PERMISSIONS.customersStatus, "permission");
  ...
  // Keyed on the active workspace, same as `UsersPage`: `listCustomers` is scoped to it
  // automatically via `bindActiveWorkspace`, so switching workspaces has to refetch or the table
  // would keep showing one workspace's customers under another's name.
    ...
    () => [
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Link to={`/customers/${customer.id}`} className="flex items-center gap-2 hover:underline">
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.dob ? (
            <span className="text-xs text-muted-foreground">{format(new Date(row.original.dob), "dd MMM yyyy")}</span>
          ) : (
      ...
      },
      {
        ...
        cell: ({ row }) => <span className="text-xs text-muted-foreground">{format(new Date(row.original.joinedDate), "dd MMM yyyy")}</span>,
      },
      {
        id: "emailVerified",
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex justify-end items-center gap-1.5">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link to={`/customers/${customer.id}`} className={cn(buttonVariants({ variant: "outline", size: "icon" }))}>
                    <EyeIcon />
                  </Link>
                </TooltipTrigger>
              ...
              <Tooltip>
                ...
                <TooltipContent>{canManage ? "Delete customer" : `You need the "${PERMISSIONS.customersManage}" permission to do this.`}</TooltipContent>
              </Tooltip>
            </div>
          );
        },
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {canManage ? (
            <Link to="/customers/new" className={cn(buttonVariants({ size: "sm" }))}>
              <PlusIcon />
              Add customer
            </Link>
          ) : null}
        ...
        <CardContent className="flex flex-col gap-4">
          {isError ? <p className="text-sm text-destructive">{apiErrorMessage(error, "Couldn't load customers. Try again.")}</p> : null}
