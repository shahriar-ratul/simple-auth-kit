...
export default observer(function CountriesPage() {
  ...
  // `activeWorkspaceId` is part of the query key, same reasoning as the users list: countries are
  // workspace-scoped, so switching workspace has to re-fetch rather than leave the previous
  // workspace's countries on screen.
  ...
  const columns = useMemo<ColumnDef<CountrySummary>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Link href={`/countries/${row.original.id}`} className="font-medium hover:underline">
            {row.original.name}
          </Link>
        ),
      },
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex justify-end items-center gap-1.5">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link href={`/countries/${country.id}`} className={cn(buttonVariants({ variant: "outline", size: "icon" }))}>
                    <EyeIcon />
                  </Link>
                </TooltipTrigger>
              ...
              <Tooltip>
                ...
                <TooltipContent>{canManage ? "Delete country" : missingPermissionHint(PERMISSIONS.countriesManage)}</TooltipContent>
              </Tooltip>
            </div>
          );
        },
  ...
  if (!canRead) return <PermissionRequired permission={PERMISSIONS.countriesRead} what="Countries" />;
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          {canManage && (
            <Link href="/countries/new" className={cn(buttonVariants({ size: "sm" }))}>
              <PlusIcon />
              Add country
            </Link>
          )}
        ...
        <CardContent className="flex flex-col gap-4">
          {isError && <p className="text-sm text-destructive">{error instanceof AuthApiError ? error.message : "Couldn't load countries. Try again."}</p>}
